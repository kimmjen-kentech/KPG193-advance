...
9. **근거 없는 단정 금지** — "가장 일반적", "표준", "흔히 쓰임"처럼 검증되지 않은 권위 표현을 쓰지 않는다. 도메인 용어·관례·통계를 단언할 때는 (a) 실제 출처를 제시하거나, (b) "확인 필요", "추측"이라고 명시하거나, (c) 사용자(도메인 전문가)에게 묻는다. 추측을 사실처럼 말하는 것은 거짓말이다.
